...
class RemoteMixtureOfExperts(nn.Module):
    ...
    def outputs_schema(self):
        if self._outputs_schema is None:
            ...
            dummy_scores_flat = self.gating_function(torch.randn(1, self.gating_function.in_features))
            dummy_scores = dummy_scores_flat.split_with_sizes(self.grid_size, dim=-1)
